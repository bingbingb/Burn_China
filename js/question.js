...
function rebuildQuestionTbl(allQuestions) {
   ...
   for (let qIndex = 0; qIndex < allQuestion.length; qIndex++) {
        ...
        } else if((Tname.length>25) && (Tname.length<=50)){
            allQuestion.eq(qIndex).parent(".tiMu").eq(0).css({
                "width": "calc(45%)",
                "paddingTop": "calc(2%)",
                "height": "80px",
		...
		else if((Tname.length>50) && (Tname.length<100)){
            allQuestion.eq(qIndex).parent(".tiMu").eq(0).css({
                "width": "calc(50%)",
                "paddingTop": "calc(2%)",
                "height": "100px",
    ...
    for (let eIndex = 0; eIndex < allElect.length; eIndex++) {
        ...
        } else if ((Ename.length > 12) && (Ename.length <= 20)) {
            allElect.eq(eIndex).css({"width": "calc(60%)", "border-radius": "30px", "paddingLeft": "10px"});
        } else if ((Ename.length > 25) && (Ename.length <= 29)) {
...
function showAnalysis() {
    ...
    for (let qIndex = 0; qIndex < questionDom.length; qIndex++) {
        let tempName = answer[qIndex].type;
        let str = "<div class='analyzeDv'><div>" + "解析：" + tempName + "</div></div>";
...
function analyseOptionForm(question, qid) {
    ...
    for (var optionIndex in allOptions) {
        ...
        optionBody += "<div class='elect'>" + "<label><input type=" + optionType + " name=" + qid + " optionName=" + optionName + "\
                                        ><p class='Elect'>" + allOptions[optionIndex].label + "、" + allOptions[optionIndex].name + "</p></label></div>";
    }
    return optionBody;
}
...
function showtt(text) {
    ...
    if (text < 60) {
        ...
        pzi = '<p class="alertWen">' + "本次成绩：" + '<span class="fenWen">'
            + text + "分" + '</span>' + "<br/>" + img + '</p><p class="tanWen">' + "不要灰心,<br/>继续努力！" + '</p>';
    } else if (60 < text <= 95) {
        var img = '<img class="bimg" src="img2/80分.png" alt=""/>';
        pzi = '<p class="alertWen">' + "本次成绩：" + '<span class="fenWen">'
            + text + "分" + '</span>' + "<br/>" + img + '</p><p class="tanWen">' + "继续努力，<br/>下一个学霸就是你！" + '</p>';
    } else if (text === 100) {
        var img = '<img class="bimg" src="img2/100分.png" alt=""/>';
        pzi = '<p class="alertWen">' + "本次成绩：" + '<span class="fenWen">'
            + text + "分" + '</span>' + "<br/>" + img + '</p><p class="tanWen">' + "恭喜学霸诞生！" + '</p>';
    }
